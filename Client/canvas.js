export class CanvasManager {
  ...
  setMode(mode) {
    this.mode = mode;
    // update user's drawing context immediately so local strokes match the mode
    if (this.userCtx) {
      if (this.mode === 'eraser') {
        this.userCtx.globalCompositeOperation = 'destination-out';
        this.userCtx.strokeStyle = 'rgba(0,0,0,1)';
  ...
  setColor(color) {
    ...
    // apply to user's drawing context immediately so local strokes use new color
    if (this.userCtx && this.mode !== 'eraser') {
      this.userCtx.strokeStyle = color;
    }
  ...
  setLineWidth(width) {
    ...
    // update user's context line width so new strokes use updated size
    if (this.userCtx) this.userCtx.lineWidth = width;
